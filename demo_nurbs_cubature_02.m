...
% The variable "domain_type" set the domains taken into account.
if nargin < 1
    example=1;
end
...
ADEref=30;
xywL = cubRS(ADEref,geometry_NURBS_EXT,geometry_NURBS_INT,0,Nbox,safe_mode);
...
for jj=1:length(adeV)
    ...
    %---
    [xyw,res,Z,Zin] = cubRS(ade,geometry_NURBS_EXT,geometry_NURBS_INT,...
...
for k=1:size(Zcards,1)
    fprintf('\n \t %2.0d | %4d | %4d  | %4d  | ',adeV(k),Zcards(k,1),...
